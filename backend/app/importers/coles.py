...
class ColesImportSource(SupermarketImportSource):
    store_code = "coles"

    def __init__(self):
        super().__init__(source_name="coles")

    def normalize_item(self, item: dict) -> ProductImportRecord:
        return self._wrap_single_offer(
            item,
            canonical_name=item.get("name") or item.get("title") or "Unknown Coles Product",
