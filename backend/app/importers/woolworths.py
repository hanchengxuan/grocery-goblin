...
class WoolworthsImportSource(SupermarketImportSource):
    store_code = "woolworths"

    def __init__(self):
        super().__init__(source_name="woolworths")

    def normalize_item(self, item: dict) -> ProductImportRecord:
        return self._wrap_single_offer(
            item,
            canonical_name=item.get("name") or item.get("title") or "Unknown Woolworths Product",
